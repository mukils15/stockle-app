...
let wordle;
const getWordle = ()=>{
    fetch('http://localhost:8000/word').then((response)=>response.json()
    ).then((json)=>{
...
document.addEventListener('keydown', (event)=>{
    if (event.key.toUpperCase() === 'BACKSPACE') handleClick('«');
    else if (keys.includes(event.key.toUpperCase())) handleClick(event.key.toUpperCase());
});
...
const checkRow = ()=>{
    ...
    if (currentTile > 4) fetch(`http://localhost:8000/check/?word=${guess}`).then((response)=>response.json()
    ).then((json)=>{
...
const showMessage = (message)=>{
    ...
    setTimeout(()=>messageDisplay.removeChild(messageElement)
    , 10000);
};
const loseMessage = (message)=>{
    ...
    for(let g = 0; g <= currentRow; g++){
        for(let j = 0; j < 5; j++){
            if (final_Rows[g][j] === 'b') str_cont += '⬛ ';
            else if (final_Rows[g][j] === 'bl') str_cont += '🟦 ';
            else str_cont += '🟩 ';
        }
        str_cont += '\n';
    }
    ...
    setTimeout(()=>messageDisplay.append(outerDiv)
    , 3000);
};
